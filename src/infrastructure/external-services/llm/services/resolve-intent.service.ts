/**
 * @file resolve-intent.service
 * @description Service to resolve user intent and extract structured data using OpenAI.
 */
...
import { RESOLVE_INTENT_PROMPT } from '@/infrastructure/external-services/llm/prompts/resolve-intent.es.prompt';
import { extractJsonFromString } from '@/shared/utils/extractJsonFromString';

/**
 * Service for resolving user intent from messages using LLM.
 */
@Injectable()
export class ResolveIntentService implements ResolveIntentPort {
    ...
    /**
     * Resolves intent from user message using LLM and prompt in Spanish.
     * @param message The normalized user message.
     * @param context Additional date/time context for prompt replacement.
     * @returns Object with detected intent type and payload.
     */
